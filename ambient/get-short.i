 ...
 * FOR EACH Customer:
 * ix = GET-SHORT(RAW(Customer.Name), 1).
 * IF ix = 83 THEN
 * DISPLAY Customer.Name.
 * END.
 * ```
 *
 * @param source A function or variable that returns a RAW or MEMPTR value. If source is the Unknown value (?), GET-SHORT returns the Unknown value (?).
 ...
 * @returns INTEGER
 */
FUNCTION GET-SHORT RETURNS INTEGER (
  source AS RAW,
  position AS INTEGER
  ) FORWARD.
